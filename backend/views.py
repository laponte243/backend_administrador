...
def actualizar_pedido(request):
    ...
    if verificar_permiso(perfil, 'Pedido', 'actualizar'):
        ...
        try:
            ...
            pedido.total=total
            pedido.save()
            return Response({'total_pedido':total, 'info':info}, status=status.HTTP_200_OK)
        except Exception as e:
            ...
            if verificar_permiso(perfil, 'Comisiones', 'leer'):
                ...
                for n in notas:
                    ...
                    for d in detalle:
                        ...
                        excel_ws.write(i, 3, '%s%s' % (correlativo_prof.prefijo + '-' if correlativo_prof.prefijo else '', d.proforma.numerologia))
...
def calcular_credito(request):
    ...
    if user:
        ...
        for pro in Proforma.objects.filter(cliente=cliente):
            ...
            p.append({'doc':'proforma', 'fecha':pro.fecha_proforma, 'pre_doc':correlativo_pro.prefijo if correlativo_pro.prefijo else '', 'num_doc':pro.numerologia, 'total':pro.total, 'saldo':pro.saldo_proforma, 'fecha_b': pro.fecha_despacho.date() if pro.fecha_despacho else '', 'factura':factura.values_list('id') if factura else None})
