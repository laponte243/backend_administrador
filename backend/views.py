...
class LoginView(KnoxLoginView):
    ...
    def post(self, request, format = None):
        ...
        # Funcion para marcar al sistema el inicio de sesion
        login(request, user)
        # Crear respuesta
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        try:
            if Token.objects.get(key = kwargs['token']):
                ...
                value = {'data': []}
                agrupador = DetallePedido.objects.filter(pedido = pedido).values('id', 'producto').annotate(total = Sum('total_producto'), cantidad = Sum('cantidada'))
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        try:
            if Token.objects.get(key = kwargs['token']):
                ...
                total_imponible = 0
                total_calculado = 0
                agrupador = DetalleProforma.objects.filter(proforma = proforma).values('id', 'producto', 'precio').annotate(total = Sum('total_producto'), cantidad = Sum('cantidada'))
                # Ciclo para generar Json y data para el template
                for dato in agrupador:
                    # Obtener producto
                    productox = Producto.objects.get(id = dato['producto'])
    ...
    if verificar_permiso(perfil, 'Notasdepago', 'borrar'):
        ...
        try:
            ...
            for obj in DetalleNotasPago.objects.filter(notapago = nota):
                ...
                proforma.saldo_proforma = Decimal(float(proforma.saldo_proforma)) + Decimal(float(obj.monto))
                proforma.save()
                obj.delete()
            nota.delete()
            ...
            for obj in payload['data']:
                try:
                    if obj['monto'] > 0:
                        ...
                        detalle_nota = DetalleNotasPago(instancia = instancia, proforma = proforma, notapago = nota_pago, monto = Decimal(obj['monto']), saldo_anterior = Decimal(obj['saldo_anterior']))
                        detalle_nota.save()
        ...
        try:
            ...
            for d in request.data['detalles']:
                try:
                    ...
                    precio_seleccionado = Decimal(float(d['precio_seleccionado']))
                    cantidad = int(d['cantidada'])
                    # Calcular el precio de cada producto segun la cantidad
                    total_producto = Decimal(round(cantidad * precio_seleccionado,3))
                    ...
                    nuevo_detalle = DetallePedido(
                        ...
                        iva = Decimal(total_producto*Decimal(16.0/100.0)) if not producto.exonerado else Decimal(0.0),
                    ...
                    if inventario:
                        ...
                        if retornado:
                            raise Exception(retornado)
                    if nuevo_detalle.iva:
                        imponible += (nuevo_detalle.total_producto + nuevo_detalle.iva)
                    else:
            ...
            pedido.total = total
            pedido.save()
            return Response({'total_pedido': round(exento + imponible,2), 'info': info}, status = status.HTTP_200_OK)
        except Exception as e:
...
def crear_objeto_factura(request):
    ...
    if verificar_permiso(perfil, 'Factura', 'escribir'):
        try:
            ...
            nueva_factura.direccion_empresa = empresa.direccion
            nueva_factura.formato_grande = empresa.factura_grande
            ...
            if imponible:
                # Sumar al total
                total_real = round(total_real, 2) + (round(imponible, 2)*impuesto)
...
def vista_xls(request):
    ...
    if Token.objects.get(key = token):
        ...
        if verificar_permiso(perfil, 'Productos', 'leer'):
            ...
            excel_ws.write(i, 4, 'PRECIO1', estilo)
            excel_ws.write(i, 5, 'PRECIO2', estilo)
            excel_ws.write(i, 6, 'PRECIO3', estilo)
            excel_ws.write(i, 7, 'PRECIO4', estilo)
            i = i + 1 # Saltar fila
...
@api_view(["GET"])
@csrf_exempt
def lista_precio_xls(request):
    data = request.data
    if not data.get('marcas', None):
        data['marcas'] = Marca.objects.all().values('id')
    params = request.query_params.copy()
    # token = params.get('token').split(' ')[1]
    # if Token.objects.get(key = token):
    perfil = Perfil.objects.get(usuario = 1)
    if verificar_permiso(perfil, 'Productos', 'leer'):
        ...
        excel_ws.write(i, 1, 'CODIGO', estilo)
        excel_ws.write(i, 2, 'DESCRIPCIÓN', estilo)
        excel_ws.write(i, 3, 'PRECIO1', estilo)
        excel_ws.write(i, 4, 'PRECIO2', estilo)
        excel_ws.write(i, 5, 'PRECIO3', estilo)
        excel_ws.write(i, 6, 'PRECIO4', estilo)
        i = i + 1 # Saltar fila
        # Ciclo por cada marca
        for m in data['marcas']:
            m = Marca.objects.get(id = m['id'])
            # Ciclo por cada producto
            pro = Producto.objects.filter(marca = m.id)
            for p in pro.values():
                # Escribir productos
                excel_ws.write(i, 0, m.nombre) # Nombre marca
                excel_ws.write(i, 1, p['sku']) # Codigo producto
                excel_ws.write(i, 2, p['nombre']) # Nombre producto
                # Precios del producto
                excel_ws.write(i, 3, p['precio_1'])
                excel_ws.write(i, 4, p['precio_2'])
                excel_ws.write(i, 5, p['precio_3'])
                excel_ws.write(i, 6, p['precio_4'])
                i = i + 1 # Saltar fila
        # Guardar excel en el archivo temporal
        excel_wb.save(response)
        # Restornar archivo
    ...
    if user:
        ...
        try:
            if verificar_permiso(perfil, 'Comisiones', 'leer'):
                ...
                excel_ws.col(6).width = 4000 # Tamaño columna total exento proforma
                excel_ws.col(7).width = 3500 # Tamaño columna total iva proforma
                excel_ws.col(8).width = 3500 # Tamaño columna monto pagado
                excel_ws.col(11).width = 4000 # Tamaño columna monto pagado
                estilo = easyxf('font: bold 1')
                ...
                excel_ws.write(i, 6, 'Sub-Total', estilo)
                excel_ws.write(i, 7, 'IVA', estilo)
                excel_ws.write(i, 8, 'Total', estilo)
                excel_ws.write(i, 9, 'Monto pagado', estilo)
                excel_ws.write(i, 10, 'Comision', estilo)
                excel_ws.write(i, 11, 'Estado',estilo)
                excel_ws.write(i, 12, 'Dias diferencia',estilo)
                i = i + 1
                total_comision = 0
                # Creador de filas
                for n in notas:
                    ...
                    for d in detalle:
                        ...
                        if proforma.fecha_despacho:
                            ...
                            excel_ws.write(i, 11, '%s' % ('Despachado'))
                        else:
                            excel_ws.write(i, 11, '%s' % ('Sin despachar'))
                        excel_ws.write(i, 10, '%s' % (comision))
                        # Dias diferencia
                        excel_ws.write(i, 12, '%s' % (dias_diferencia))
                        # excel_ws.write(i, 6, '%s' % ())
                        i = i + 1
                excel_ws.write(i, 9, '%s' % ('Total: '), estilo)
                excel_ws.write(i, 10, '%s' % (round(total_comision, 2)))
                excel_wb.save(response)
                return response
            else:
                return Response(status = status.HTTP_401_UNAUTHORIZED)
...
def calcular_credito(request):
    ...
    if user:
        ...
        # Iniciar Excel
        response = HttpResponse(content_type = 'application/ms-excel')
        response['Content-Disposition'] = 'attachment;filename = "debito_%s.xls"' % (cliente.nombre)
        excel_wb = Workbook(encoding = 'utf-8')
        excel_ws = excel_wb.add_sheet('Credito') # ws es Work Sheet
        # Iniciar valores del Excel
        i = 0 # Salto de linea
        total = 0 # Deuda total del cliente
        saldo_pendiente = 0 # Deuda total del cliente
        # Añadiendo estilo
        estilo = easyxf('font: bold 1') # Estilo especial a campos y casillas importantes
        excel_ws.col(1).width = 10000 # Tamaño columna total proforma
        excel_ws.col(4).width = 6000 # Tamaño columna total proforma
        excel_ws.col(5).width = 6000 # Tamaño columna monto pagado
        # Definir cliente
        ...
        excel_ws.write(i, 3, 'Saldo', estilo)
        ...
        for index, row in data_excel.iterrows():
            ...
            if row.saldo  != None:
                excel_ws.write(i, 3, row.saldo if row.saldo else Decimal(0.0), number_style)
            else:
                excel_ws.write(i, 3, '', number_style)
            if row.fecha_b:
                excel_ws.write(i, 4, '%s%s' % (row.fecha_b, ' (Despacho)' if row.doc  == 'proforma' else ' (Comprobante)'))
            else:
    ...
    for index, row in df.iterrows():
        if row['CODIGO']:
            ...
            try:
                ...
                pass
            marca, created = Marca.objects.get_or_create(nombre__exact = row['MARCA'], defaults = {'instancia': instancia, 'nombre': row['MARCA']})
            marca.save()
            exonerado = True
...
#     for p in productos:
#         encontrado = Producto.objects.filter(codigo = p.codigo).exclude(id = p.id)
